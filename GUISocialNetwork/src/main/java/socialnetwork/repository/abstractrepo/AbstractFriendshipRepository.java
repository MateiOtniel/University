...
public abstract class AbstractFriendshipRepository implements Repository<Friendship> {
    ...
    public void add(Friendship friendship) throws NetworkException {
        ...
        network.get(id2).add(new Friendship(id2, id1, friendship.getTime(), 0));
        save();
    }
    ...
    public void acceptRequest(int id1, int id2) {
        if (network.containsKey(id1)) {
            for(int i = 0; i < network.get(id1).size(); i++)
                if(network.get(id1).get(i).getTo() == id2)
                    network.get(id1).get(i).setState(1);
        }
        if (network.containsKey(id2)) {
            for(int i = 0; i < network.get(id2).size(); i++)
                if(network.get(id2).get(i).getTo() == id1)
                    network.get(id2).get(i).setState(1);
        }
        save();
    }
}
