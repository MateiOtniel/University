...
public class DBFriendshipRepository extends AbstractFriendshipRepository {
    ...
    public void read() {
        try {
            ...
            while (resultSet.next()) {
                ...
                network.get(from).add(new Friendship(from, to, LocalDateTime.parse(date), state));
            }
    ...
    private void addDataDB(Connection connection, PreparedStatement ps) {
        try {
            ps = connection.prepareStatement("INSERT INTO network (id1, id2,time,state) VALUES (?,?,?,?)");
