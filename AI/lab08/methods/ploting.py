...
def plotNormalisedData(inputs, outputs, outputNames):
	...
	plotClassificationData(feature1train, feature2train, trainOutputs, outputNames,
	                       'normalised train data')
	return trainInputs, trainOutputs, feature1test, feature2test, testInputs, testOutputs


def plotPredictions(outputs, feature1, feature2, realOutputs, computedOutputs, title,
                    labelNames):
	labels = list(set(outputs))
	noData = len(feature1)
	for crtLabel in labels:
		x = [feature1[i] for i in range(noData) if
		     realOutputs[i] == crtLabel and computedOutputs[i] == crtLabel]
		y = [feature2[i] for i in range(noData) if
		     realOutputs[i] == crtLabel and computedOutputs[i] == crtLabel]
