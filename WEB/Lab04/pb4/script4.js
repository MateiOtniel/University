function sorteazaColoana(lineNumber) {
    let table = $("#tabel");
    ...
    for (let index = 1; index < rows.length; index++) {
        ...
        toAdd.sortValue = $(rows[index]).children().eq(lineNumber).find("div").html();
        toSort.push(toAdd);
    }
...
function sorteazaLinia(indexLinie) {
    ...
    for (let index = 0; index < rows.length; index++) {
        for (
            let nthChild = 1;
            nthChild < rows.eq(index).children().length;
            nthChild++
        ) {
            columns[nthChild - 1].push(
                rows.eq(index).children().eq(nthChild).find("div").html()
            );
        }
    }
    ...
    for (let index = 0; index < rows.length; index++) {
        for (
            let nthChild = 0;
            nthChild < rows.eq(index).children().length - 1;
            nthChild++
        ) {
            rows
            .eq(index)
