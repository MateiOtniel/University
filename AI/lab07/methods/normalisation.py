...
def normalisation(trainInputs, testInputs):
	...
	if not isinstance(trainInputs[0], list):
		...
		normalisedTrainData = scaler.transform(
			trainData)  # apply same transformation to train data
		normalisedTestData = scaler.transform(
			testData)  # apply same transformation to test data
		...
		normalisedTrainData = scaler.transform(
			trainInputs)  # apply same transformation to train data
		normalisedTestData = scaler.transform(
			testInputs)  # apply same transformation to test data
