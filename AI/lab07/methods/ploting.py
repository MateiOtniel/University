...
def plotTrainingTestUnivariate(inputsPlot, outputsPlot):
	...
	plt.plot(trainInputs, trainOutputs, 'ro',
...
def plotModelBivariate(w0, w1, w2, trainOutputs, feature1, feature2, feature1Train,
                       feature2Train):
	noOfPoints = 50
	xref1 = []
	val = min(feature1)
...
def plotModelUnivariate(w0, w1, trainOutputs, trainInputs):
	...
	plt.plot(xref, yref, 'b-',
	         label='learnt model')  # the model is plotted by blue and line sign
...
def makePredictionBivariate(testInputs, testOutputs, regressor, feature1Test,
                            feature2Test):
	# computedTestOutputs = [w0 + w1 * el[0] + w2 * el[1] for el in validationInputs]
	# makes predictions for test data (by tool)
	computedTestOutputs = regressor.predict(testInputs)
	
	plot3Ddata([], [], [], feature1Test, feature2Test, computedTestOutputs,
	           feature1Test,
...
def makePredictionUnivariate(testInputs, testOutputs, regressor):
	...
	plt.plot(testInputs, testOutputs, 'ro',
	         label='real test data')  # real test data are plotted by red and circle sign
...
def plotOptional(testOutputs, predict):
	...
	plt.plot([testOutputs.min(), testOutputs.max()], [predict.min(), predict.max()],
	         color='red')
	plt.xlabel('Real')
