...
from methods.ploting import plot3Ddata, plotTrainingTestBivariate, plotModelBivariate, \
	makePredictionBivariate, \
	plotTrainingTestUnivariate, plotModelUnivariate, makePredictionUnivariate, \
	plotOptional
import warnings
...
def loadData(fileName, inputVariableName1, inputVariableName2, outputVariabName):
	...
	inputs = {inputVariableName1: [float(data[i][selectedVariable1]) for i in
	                               range(len(data))],
	          inputVariableName2: [float(data[i][selectedVariable2]) for i in
	                               range(len(data))]}
	selectedOutput = dataNames.index(outputVariabName)
	outputs = {
		outputVariabName: [float(data[i][selectedOutput]) for i in range(len(data))]}
...
if __name__ == '__main__':
	...
	functions = [learnModel(trainInputsList, trainOutputs, 0),
	             toolLearning(trainInputsList, trainOutputs, 0),
	             toolLearningAdapted(trainInputsList, trainOutputs,
	                                 0)]  # 0 for univariate
	...
	feature1Train, feature2Train, trainInputs, trainOutputs, feature1Test, feature2Test, \
		testInputs, testOutputs = plotTrainingTestBivariate(feature1, feature2,
	                                                        outputs)
	functions2 = [learnModel(trainInputs, trainOutputs, 1),
	              toolLearning(trainInputs, trainOutputs, 1),
	...
	for i in range(len(functions2)):
		...
		x1ref, x2ref, yref = plotModelBivariate(w0, w1, w2, trainOutputs, feature1,
		                                        feature2, feature1Train, feature2Train)
		error = makePredictionBivariate(testInputs, testOutputs, regressor,
		                                feature1Test, feature2Test)
		text2 = 'Function: ' + names[
			i] + '\nGDP & Freedom -> the learnt model: f(x) = ' + str(
			w0) + ' + ' + str(w1) + '* x1 + ' + str(
			w2) + ' * x2 \nerror = ' + str(error) + '\n\n'
	...
	trainInputs, testInputs, trainOutputs, testOutputs = train_test_split(X, Y,
	                                                                      test_size=0.2,
	                                                                      random_state=37)
