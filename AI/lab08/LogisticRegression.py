...
class OneLogisticRegression:
	...
	def fit(self, x, y, learningRate=0.5, noEpochs=1000):
		self.coef_ = [0.0 for _ in range(1 + len(x[0]))]
		# beta or w coefficients y = w0 + w1 * x1 + w2 * x2 + ...
		for epoch in range(noEpochs):
			...
			for i in range(len(x)):  # for each sample from the training data
				...
				for j in range(0, len(x[0])):  # update the coefficients
					self.coef_[j + 1] = self.coef_[j + 1] - learningRate * crtError * \
					                    x[i][j] * ycomputed * (1.0 - ycomputed)
